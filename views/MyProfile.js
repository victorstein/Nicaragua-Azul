...
import { View, Alert, Text, Image, StatusBar, ImageBackground, ActivityIndicator, AsyncStorage, ScrollView, TouchableOpacity, Share } from 'react-native';
...
export default class MyProfile extends Component {
            ...
            Alert.alert(
              ...
              [
                ...
                {text: 'OK', onPress: async () => {
                  ...
                  await this.setState({ data: data, isUserLoggedIn: true, loadingfb: false, treasuresTotal: treasures, embajador: data.embajador });
                }},
              ],
              { cancelable: false }
            )
  ...
  async componentDidMount(){
    ...
    if (isUserLoggedIn.provider !== null){
      this.setState({ loading: false, isUserLoggedIn: true, data: isUserLoggedIn, treasuresTotal: treasuresTotal, embajador: isUserLoggedIn.embajador })
    }
    else{
  ...
  congrats = ()=>{
    return(
      <View>
        <Text style={{ color: "#fff", fontSize: 16, paddingHorizontal: 20, marginVertical: 25, textAlign: 'center'  }}>
      ...
      >
        ...
        <View style={ this.buttonStyle() }>
          <Button
            ...
            fontWeight="600"
            fontSize={22}
            containerViewStyle={{ bottom: -25, zIndex: 10, }}
            onPress={ async ()=>{
              const url = 'nicaraguazul'
              Share.share({
        ...
        <View style={{ height: (helper.height < 540) ? helper.height/1.5 : helper.height/2 , backgroundColor: "#27A9FF", alignItems:"center" }}>
          <View style={{ flexDirection: 'row', marginTop: 50,}}>
            <View>
              <View style={{ backgroundColor: "white", height: 100, width: 100, borderRadius: 100, alignItems:"center", justifyContent: 'center', marginHorizontal: 30 }}>
                <Text style={{ color: "#37AFFF", fontSize: 24 }}>{ (this.state.data.treasures["0"] == "none") ? 0 : Object.keys(this.state.data.treasures).length }/{ this.state.treasuresTotal }</Text>
              </View>
              <Text style={{ color: "#fff", fontSize: 24, textAlign: 'center', marginTop: 10 }}>Tesoros</Text>
            </View>
            <View>
              <View style={{ backgroundColor: "white", height: 100, width: 100, borderRadius: 100, alignItems:"center", justifyContent: 'center', marginHorizontal: 30 }}>
                <Text style={{ color: "#37AFFF", fontSize: 24 }}>{ (this.state.data.treasures["0"] == "none") ? 0 : Number(Math.round( (Object.keys(this.state.data.treasures).length)*100)/this.state.treasuresTotal).toFixed(0) }%</Text>
              </View>
