...
export default class AguaDulce extends Component {
  ...
  static navigationOptions ({ navigation }) {
    return {
      ...
      />,
      headerTitle: "Agua Dulce",
      headerStyle: { backgroundColor: "#03A9F4", height: 70, margin: 0, marginTop: -Constants.statusBarHeight },
  ...
  async componentDidMount(){
    ...
    let wiki = [];
    Object.entries(data.AguaDulce).forEach((u, i)=>{
      let C = u[0];
          ...
          if("Biodiversidad" in item){
            return(
              <TouchableOpacity
                ...
                onPress={()=>{
                  this.props.navigation.navigate('RenderBioView', {data: 'AguaDulce', header: 'Biodiversidad', headerBackgroundColor: '#03A9F4'} );
                }}
              >
                <View style={{ width: '80%', justifyContent: 'center', paddingLeft: 30 }}>
                  <Text style={{ fontSize: 18, }}>Biodiversidad</Text>
                  <Text style={{ fontSize: 14, color: '#BCBEBF' }}>AguaDulce</Text>
                </View>
            ...
            return(
              <TouchableOpacity
                style={{ flexDirection: 'row', width: '100%', height: helper.height/8 }}
                onPress={()=>{
                  this.props.navigation.navigate('RenderBioView', {data: 'AguaDulce', header: 'Especies Migratorias e Introducidas de Agua Dulce', headerBackgroundColor: '#03A9F4'} );
                }}
              >
                <View style={{ width: '80%', justifyContent: 'center', paddingLeft: 30 }}>
                  <Text style={{ fontSize: 18, }}>Especies Migratorias e Introducidas de Agua Dulce</Text>
                  <Text style={{ fontSize: 14, color: '#BCBEBF' }}>AguaDulce</Text>
                </View>
                <View style={{ width: '20%', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon
